...
class LostForm(Page):
    def __init__(self):
        Page.__init__(self, "lost_form", html_file="lost_form/index", bypass=False, public=False)

    def get(self, event=None, path=None, post={}, error_msg=None, user=None):
        if not path.get("case_type"):
            return redirect("lost")
        if not path.get("case_model"):
            return redirect("lost_model/" + path["case_type"])
        ...
        # if post.get("photo_pet"):
        #     html.esc("photo_pet_val", post["photo_pet"].title())
        if post.get("case_name"):
            html.esc("case_name_val", post["case_name"].title())
            html.esc("if_case_name_exists", "focus")
    ...
    def post(self, event=None, path=None, post={}, error_msg=None, user=None):
        ...
        if not post.get("case_name"):
            return self.get(event, path, post, "é necessário informar o nome do seu pet", user)
        ...
        return redirect("lost_map/" + new_case.case_id)
...
def format_case_data_params(path, post):
    ...
    filtered_post["case_name"] = post.get("case_name").lower().strip()
